...
def get_model(sample_batch, config, args):
    ...
    if args.model == "gin":
        ...
        model = RGCN(
            ...
            edge_update=args.emlps, dropout=config.dropout, final_dropout=config.final_dropout, n_bases=8
        )
    else:
        raise ValueError(f"Unknown model type: {args.model}. Choose from: gin, gat, pna, rgcn")
