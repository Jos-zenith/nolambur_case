...
def get_data(args, data_config):
    ...
    i,j = min(split_scores, key=lambda x: split_scores[x])
    ...
    if not args.model == 'rgcn':
        ...
        val_data.edge_attr = z_norm(val_data.edge_attr) if val_data.edge_attr is not None else None
        te_data.edge_attr = z_norm(te_data.edge_attr) if te_data.edge_attr is not None else None
    else:
        if tr_data.edge_attr is not None and val_data.edge_attr is not None and te_data.edge_attr is not None:
