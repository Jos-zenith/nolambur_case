...
def logger_setup():
    ...
    # File handler should use UTF-8 so files store unicode correctly
    file_handler = logging.FileHandler(os.path.join(log_directory, "logs.log"), encoding='utf-8')
    # Wrap stdout to avoid UnicodeEncodeError on terminals with limited encodings
    try:
